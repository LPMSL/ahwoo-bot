...
async def log_conversation(
    ...
    row = [
        ...
        intent,
        intent_zh,
        "是" if auto_reply else "否",
        (auto_reply or "")[:300],
        "是" if needs_human else "否",
        priority,
        reason,
        round(confidence, 2),
        total_turns,
        "是" if used_static else "否",
        "是" if telegram_ok else "否",
    ]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if code in (429, 500, 503) and attempt < max_retries - 1:
                delay = 2 ** (attempt + 1)
                logger.warning(f"Google Sheets 寫入暫時錯誤 [{code}]，{delay}s 後重試 (第{attempt+1}次)")
...
async def get_unanswered_conversations(hours_threshold: int = 4) -> list:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if code in (429, 500, 503) and attempt < max_retries - 1:
                delay = 2 ** (attempt + 1)
                logger.warning(f"get_unanswered_conversations 暫時錯誤 [{code}]，{delay}s 後重試 (第{attempt+1}次)")
                await asyncio.sleep(delay)
                continue
...
async def get_daily_stats(date_str: str | None = None) -> dict:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if code in (429, 500, 503) and attempt < max_retries - 1:
                delay = 2 ** (attempt + 1)
                logger.warning(f"Google Sheets 查詢暫時錯誤 [{code}]，{delay}s 後重試 (第{attempt+1}次)")
                await asyncio.sleep(delay)
                continue
